...
namespace PostEnot.Toolkits.StateMachines.FSM
{
    internal sealed class StateCollection<TStateBase, TContext>
        : IReadOnlyCollection<TStateBase>
        where TStateBase : StateBase<TStateBase, TContext>
    {
        ...
        public bool InitializeState<TState>(StateBase<TStateBase, TContext>.MessageHandlerBase messageHandler) where TState : TStateBase, new()
        {
            Type type = typeof(TState);
            ...
            TState state = new();
            messageHandler.HandleOnInit(state);
            _states.Add(type, state);
            return true;
        }

        public TStateBase GetOrCreateState<TState>(StateBase<TStateBase, TContext>.MessageHandlerBase messageHandler) where TState : TStateBase, new()
        {
            Type type = typeof(TState);
